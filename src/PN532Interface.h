...
public:
    ...
    *           not 0   failed
    */
    virtual int8_t writeCommand(const uint8_t *header,const uint8_t hlen,const uint8_t *body,const uint16_t blen) = 0;
    /**
    * @brief    write a command and check ack
    * @param    header  packet header
    * @param    hlen    length of header
    * @param    bodylist    pointer list of body data
    * @param    blenlist    list of each body length
    * @param    bodycount   number of body datas
    * @return   0       success
    *           not 0   failed
    */
    virtual int8_t writeCommand(const uint8_t *header,const uint8_t hlen,const uint8_t **bodylist,const uint16_t *blenlist,const uint8_t bodycount) = 0;
